...
Response format:
{"intent":"record_transaction"|"query_balance"|"query_report"|"query_sales_report"|"query_customer_statement"|"query_outstanding"|"manage_invoice"|"payment_received"|"update_transaction"|"general_question"|"unknown","transaction":{"amount":number,"type":"income"|"expense","category":"string","description":"string","date":"YYYY-MM-DD","customer_name":"string|null","payment_status":"paid"|"unpaid"|"partial","paid_amount":"number|null","bill_number":"string|null"}|null,"items":[{"item_name":"string","quantity":number,"unit":"string","rate":number,"gst_rate":number}]|null,"customer_name":"string|null","invoice_action":{"action":"get"|"edit"|"delete","invoice_number":"string|null","customer_name":"string|null","updates":{"amount":"number|null","status":"paid"|"unpaid"|"partial"|null,"due_date":"YYYY-MM-DD|null"}|null}|null,"reply":"string"}
...
- query_sales_report: user asks for overall sales report, sales statement, total sales, or "show all sales", "sales data", "sales summary". No customer_name needed. This shows all sales across all customers with products, dates, payments.
...
- bill_number: For Sales/Purchase, extract any bill number, invoice number, or receipt number mentioned (e.g. "bill 1234", "bill no. 456", "invoice #789"). Set null if no bill number is mentioned. Do NOT confuse with invoice_number for the manage_invoice intent.
- items: For Sales/Purchase, if multiple items are mentioned with quantities and rates (e.g. "5 kg rice at 40, 2 kg sugar at 50"), extract each as {item_name, quantity, unit, rate, gst_rate}. Set transaction amount as total of all items. If only one item or no item details, set items to null. Default gst_rate=0, unit="pcs".
- Reply: friendly, short, use emojis

Examples:
"spent ₹500 groceries" → {"intent":"record_transaction","transaction":{"amount":500,"type":"expense","category":"Groceries","description":"Spent on groceries","date":"${new Date().toISOString().split('T')[0]}","customer_name":null,"payment_status":"paid","paid_amount":null,"bill_number":null},"customer_name":null,"invoice_action":null,"reply":"Got it! ₹500 expense under Groceries recorded. 🛒"}
"sales to Ravi 10000 rs unpaid" → {"intent":"record_transaction","transaction":{"amount":10000,"type":"income","category":"Sales","description":"Sales to Ravi","date":"${new Date().toISOString().split('T')[0]}","customer_name":"Ravi","payment_status":"unpaid","paid_amount":null,"bill_number":null},"customer_name":"Ravi","invoice_action":null,"reply":"Got it! ₹10,000 unpaid sales to Ravi recorded. 📈"}
"purchase from ABC 5000 rs paid 2000 bill 1234" → {"intent":"record_transaction","transaction":{"amount":5000,"type":"expense","category":"Purchase","description":"Purchase from ABC","date":"${new Date().toISOString().split('T')[0]}","customer_name":"ABC","payment_status":"partial","paid_amount":2000,"bill_number":"1234"},"items":null,"customer_name":"ABC","invoice_action":null,"reply":"Got it! ₹5,000 purchase from ABC recorded (₹2,000 paid, Bill #1234). 🛒"}
"sold 5 kg rice at 40 and 2 kg sugar at 50 to Ravi" → {"intent":"record_transaction","transaction":{"amount":300,"type":"income","category":"Sales","description":"Sales to Ravi - Rice, Sugar","date":"${new Date().toISOString().split('T')[0]}","customer_name":"Ravi","payment_status":"paid","paid_amount":null,"bill_number":null},"items":[{"item_name":"Rice","quantity":5,"unit":"kg","rate":40,"gst_rate":0},{"item_name":"Sugar","quantity":2,"unit":"kg","rate":50,"gst_rate":0}],"customer_name":"Ravi","invoice_action":null,"reply":"Got it! ₹300 sale to Ravi (2 items) recorded. 📦"}
"received 5000 from Ravi" → {"intent":"payment_received","transaction":{"amount":5000,"type":"income","category":"Sales","description":"Payment received from Ravi","date":"${new Date().toISOString().split('T')[0]}","customer_name":"Ravi","payment_status":"paid","paid_amount":null,"bill_number":null},"customer_name":"Ravi","invoice_action":null,"reply":"Recording payment of ₹5,000 from Ravi... 💰"}
"payment from Kumar 8000" → {"intent":"payment_received","transaction":{"amount":8000,"type":"income","category":"Sales","description":"Payment received from Kumar","date":"${new Date().toISOString().split('T')[0]}","customer_name":"Kumar","payment_status":"paid","paid_amount":null,"bill_number":null},"customer_name":"Kumar","invoice_action":null,"reply":"Recording ₹8,000 payment from Kumar... 💰"}
"update sale to Ravi amount 8000" → {"intent":"update_transaction","transaction":{"amount":8000,"type":"income","category":"Sales","description":"Updated sale to Ravi","date":"${new Date().toISOString().split('T')[0]}","customer_name":"Ravi","payment_status":"paid","paid_amount":null,"bill_number":null},"customer_name":"Ravi","invoice_action":null,"reply":"Updating Ravi's sale to ₹8,000... 📝"}
"return from Ravi 2000" → {"intent":"update_transaction","transaction":{"amount":2000,"type":"expense","category":"Sales","description":"Sales return from Ravi","date":"${new Date().toISOString().split('T')[0]}","customer_name":"Ravi","payment_status":"paid","paid_amount":null,"bill_number":null},"customer_name":"Ravi","invoice_action":null,"reply":"Processing ₹2,000 return from Ravi... 🔄"}
...
"show sales report" → {"intent":"query_sales_report","transaction":null,"customer_name":null,"invoice_action":null,"reply":"Generating sales report..."}
"sales statement" → {"intent":"query_sales_report","transaction":null,"customer_name":null,"invoice_action":null,"reply":"Fetching complete sales data..."}
...
export interface GeminiResponse {
  intent: 'record_transaction' | 'query_balance' | 'query_report' | 'query_sales_report' | 'query_customer_statement' | 'query_outstanding' | 'manage_invoice' | 'payment_received' | 'update_transaction' | 'general_question' | 'unknown'
  transaction: {
    amount: number
    type: 'income' | 'expense'
    category: string
